...
def run(credit_data, numeric_feats, category_feats):
    ...
    with timer("cal iv"):
        ...
        fig = plot_bin_woe(binx=iv_details[iv_details['variable'] == 'credit.amount'],
                           title=None,
                           display_iv=True)
        iv_details.to_csv(r'./wgcpy/result/iv_details.csv', index=False)
    ...
    with timer('cal psi'):
        ...
        psi_df = numeric_var_cal_psi(except_array,
                                     ...
                                     bucket_type='bins',
                                     detail=True)
        psi_df.to_csv(r'./wgcpy/result/psi.csv')
    ...
    with timer("cal cv score"):
        ...
        fs.result_save(output=r"./wgcpy/result/feats_seletor_result.xlsx")
    ...
    with timer("PMML model build"):
        ...
        predict = pmml_model.evaluate(data=tes_x,
                                      target="flag")
        pmml_model.persist(base_dir=r"wgcpy/result",
                           model_name="credit")


if __name__ == "__main__":
    # 数据读取
    credit_data = pd.read_csv(r'wgcpy/data/germancredit.csv')
